...
learning_rate = 0.0001  # 학습률을 더 작게 설정
gamma = 0.99  # 할인율
...
for episode in range(num_episodes):
    ...
    for step in range(100):  # 각 에피소드에서 최대 100 스텝
        ...
        if np.random.random() < epsilon:
            ...
            logits = logits / torch.max(torch.abs(logits))  # logits 정규화
            probs = torch.softmax(logits, dim=-1)
    ...
    if episode % 10 == 0:
        print(f"Episode {episode}, Wire Length: {calculate_wire_length(current_cells, graph)}, Epsilon: {epsilon}")
        print(current_cells)
